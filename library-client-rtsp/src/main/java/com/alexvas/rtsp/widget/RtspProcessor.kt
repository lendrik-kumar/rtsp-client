...
class RtspProcessor(
    ...
    private var videoFrameQueue = VideoFrameQueue(5)
    ...
    private var videoWidthHint: Int? = null
    private var videoHeightHint: Int? = null
    ...
    private val proxyClientListener = object: RtspClient.RtspClientListener {
        ...
        override fun onRtspConnected(sdpInfo: SdpInfo) {
            ...
            if (sdpInfo.videoTrack != null) {
                ...
                val w = sdpInfo.videoTrack?.width ?: 0
                val h = sdpInfo.videoTrack?.height ?: 0
                if (w > 0 && h > 0) {
                    videoWidthHint = w
                    videoHeightHint = h
                }
                ...
                videoSps = sps
                videoPps = pps
                if (vps.isNotEmpty()) videoVps = vps

                // Initialize decoder
                @SuppressLint("UnsafeOptInUsageError")
                if (sps != null && pps != null) {
                    val vps: ByteArray = sdpInfo.videoTrack?.vps ?: ByteArray(0)
                    val data = ByteArray(vps.size + sps.size + pps.size)
                    var offset = 0
                    if (vps.isNotEmpty()) {
                    ...
                    videoFrameQueue.push(
                        FrameQueue.VideoFrame(
                            if (sdpInfo.videoTrack?.videoCodec == RtspClient.VIDEO_CODEC_H265) VideoCodecType.H265 else VideoCodecType.H264,
                            isKeyframe = true,
                            data,
                            0,
                            data.size,
                    ...
                    try {
                        ...
                        val spsData = NalUnitUtil.parseSpsNalUnitPayload(
                            sps, startNalOffset, sps.size - startNalOffset)
        ...
        override fun onRtspAudioSampleReceived(data: ByteArray, offset: Int, length: Int, timestampUs: Long) {
            if (DEBUG) Log.v(TAG, "onRtspAudioSampleReceived(length=$length, timestampUs=$timestampUs)")
            if (length > 0) {
            ...
            dataListener?.onRtspDataAudioSampleReceived(data, offset, length, timestampUs)
        }

        override fun onRtspApplicationDataReceived(data: ByteArray, offset: Int, length: Int, timestampUs: Long) {
            if (DEBUG) Log.v(TAG, "onRtspApplicationDataReceived(length=$length, timestampUs=$timestampUs)")
            dataListener?.onRtspDataApplicationDataReceived(data, offset, length, timestampUs)
        }
    ...
    inner class RtspThread: Thread() {
        ...
        override fun run() {
            ...
            try {
                ...
                val currentSocket = if (uri.scheme?.lowercase() == "rtsps")
                ...
                val rtspClient = RtspClient.Builder(currentSocket, uri.toString(), rtspStopped, proxyClientListener)
