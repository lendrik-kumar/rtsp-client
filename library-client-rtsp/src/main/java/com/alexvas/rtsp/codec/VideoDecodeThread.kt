                            ...
                            if (frame == null) {
                                ...
                                decoder.queueInputBuffer(inIndex, 0, 0, 0L, 0)
                                 ...
                                 if ((DEBUG || decoderLatencyRequested) && frame.isKeyframe) {
                                    ...
                                    // timestampUs is in usec, convert to msec for tracking
                                    keyframesTimestamps[frame.timestampUs / 1000L] = l
//                                  Log.d(TAG, "Added $l")
                                }
                                 // Calculate network latency (use ms units for stats)
                                 networkLatency = if (frame.capturedTimestampMs > -1)
                                     (frame.timestampUs / 1000L - frame.capturedTimestampMs).toInt()
                                 else
                                     -1
                                ...
                                if (DEBUG) {
                                 ...
                                 decoder.queueInputBuffer(inIndex, 0, frame.length, frame.timestampUs, flags)
...
//                      } while (outIndex != MediaCodec.INFO_TRY_AGAIN_LATER)
    ...
    companion object {
        ...
        private val DEQUEUE_INPUT_TIMEOUT_US = TimeUnit.MILLISECONDS.toMicros(10)
        private val DEQUEUE_OUTPUT_BUFFER_TIMEOUT_US = TimeUnit.MILLISECONDS.toMicros(10)
    }
